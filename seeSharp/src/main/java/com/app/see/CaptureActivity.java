...
import com.app.see.camera.CameraManager;
import com.app.see.camera.ShutterButton;
import com.app.see.language.LanguageCodeHelper;
import com.app.see.language.TranslateAsyncTask;

import misc.SpellChecker;
import misc.TextToSpeechConversion;
...
/*
    ...
    // Dictionary.txt
      ...
      String[] paths = new String[] { DATA_PATH, DATA_PATH + "spellChecker/" };
      ...
      String outputDir = DATA_PATH
              + "spellChecker/" + "dictionary.txt";
      Log.v("dict", outputDir);
      Toast.makeText(this, outputDir, Toast.LENGTH_LONG).show();
      if (!(new File(outputDir)).exists()) {
          Toast.makeText(this, "doesnt exist", Toast.LENGTH_LONG).show();
          try {
...
*/
      try {
          spellChecker = new SpellChecker("dictionary.txt", getApplicationContext());
      } catch (IOException e) {
          e.printStackTrace();
      }
  ...
  protected void onResume() {
    ...
    if (doNewInit) {      
      ...
      File storageDirectory = new File(DATA_PATH);
  ...
  boolean handleOcrDecode(OcrResult ocrResult) {
    ...
    String finalText = ocrResult.getText();
    // Spell Checker ========== Not needed ( slow )
      /*if(spellChecker != null) {
          finalText = spellChecker.correct(finalText);
      }
      else {
          Toast.makeText(CaptureActivity.this, "Error in spell checker", Toast.LENGTH_SHORT).show();
      }*/
      ...
      ocrResultTextView.setText(finalText);

    // Text to Speech Conversion
    // Toast.makeText(CaptureActivity.this, "Conversion TTS : " + finalText, Toast.LENGTH_SHORT).show();
    new TextToSpeechConversion(CaptureActivity.this, finalText);
